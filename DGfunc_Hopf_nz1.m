function DG = DGfunc_Hopf_nz1(x,y,omega,nu,wTr,wTi,N,symmetry,shape)
%% Computes the DG matrix for the Hopf bifurcation system (nz=1)
% Inputs:
...
[~,J]=FDFsymflextensor_ext_Hopf_trunc_nz1([w1(:);0],nu,forcing,ref,solshape,symmetry,shape); 


%% To take out correct part for nz=1
% biggest index set needed
[nx,ny,nz,nt,ninshape,comp] = countersymtensor(sizeshape_Hopf(shape),shape);
if strcmp(shape.type, 'ellHopf_nz1') % Specific for Hopf bifurcation problem with nz=1
      nz = ones(size(nz));
end
symmetry_idx = symmetry;
if (strcmp(shape.type, 'ellHopf_nz1') & symmetry == 1)
    symmetry_idx = 11;  % This ensures we include components 1 and 2 for nz=1
elseif (strcmp(shape.type, 'ellHopf_nz1') & symmetry == 25)
    symmetry_idx = 26;  % This ensures we include components 1 and 2 for nz=1
end
[symvar,~] = symmetryindicestensor_ext(nx,ny,nz,nt,comp,sizeshape_Hopf(shape),ninshape,symmetry_idx);

nz1 = (nz(symvar)==1);
% J is already computed only for nz==1 elements by the optimized function, so no need to filter J
...
%%
[~,~,~,Jquad]=FDFsymflextensor_ext_Hopf_trunc_nz1([dudnu(:);0],nu,forcing,ref,solshape,symmetry,shape);

[tilden2,~] = tildentensor(nx,ny,nz,nu);

% indices for the jacobian - filter for nz==1
njac = shapetensor(nx,ny,nz,nt,shape);
jacsymvar = (symvar & njac & (nz==1));

DJnu_1 = diag(tilden2(jacsymvar)); % First part derivative J wrt nu

% DJnu_1 is already filtered for nz==1, so no additional filtering needed
