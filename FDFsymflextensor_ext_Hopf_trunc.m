function [F,J,DFnu,Jquad,Fext,Fphase,tensors] = ...
             FDFsymflextensor_ext_Hopf_trunc(x,nu,forcing,ref,solshape,symmetry,Jshape)
...
% the phase condition in the Jacobian matrix.
...
MJ=length(find(jacsymvar))-1;
J=altzeros([MJ+1,MJ+1],nu);
J(1:MJ,1:MJ)=Jnonlinear(1:end-1,1:end-1); 
...
J(diagMJ)=J(diagMJ)+auxJ(1:end-1);
