function [F,J,DFnu,Jquad,Fext,Fphase,tensors,F_nz1] = ...
             FDFsymflextensor_ext_Hopf_trunc_nz1(x,nu,forcing,ref,solshape,symmetry,Jshape)
% Optimized version that only computes Jacobian for nz==1 elements
...
% F_nz1 is the residue filtered for only nz=1 components (without phase condition)

% Modified version of FDFsymflextensor_ext for Hopf bifurcation, excluding
% the phase condition in the Jacobian matrix. Only computes nz==1 elements.
...
% Apply ellHopf_nz1 condition for Jacobian computation
if strcmp(Jshape.type, 'ellHopf_nz1')% Specific for Hopf bifurcation problem with nz=1
    nz = ones(size(nz));
end

% Recompute symmetry indices with modified nz 
% For ellHopf_nz1: use symmetry=11 to include components 1 and 2 for nz=1, but only for Jacobian
idx_symmetry = symmetry;
if (strcmp(Jshape.type, 'ellHopf_nz1') & symmetry == 1)
    idx_symmetry = 11;  % This ensures we include components 1 and 2 for nz=1
elseif (strcmp(Jshape.type, 'ellHopf_nz1') & symmetry == 25)
    idx_symmetry = 26;  % This ensures we include components 1 and 2 for nz=1
end
...
% Compute F filtered for nz=1 components only (without phase condition)
...
F_nz1 = Fext_truncated(jacsymvar); 
...
% derivative of the nonlinear part in omega - for nz==1 only
Jnonlinear=Dnonlinear_nz1(w,Dw,Mw,DMw,N,Jshape,idx_symmetry);
...
function JJ=Dnonlinear_nz1(w,Dw,Mw,DMw,N,Jshape,symmetry)
% determines the Jacobian of the nonlinear part - FOR NZ==1 ONLY

  % initialize indices - use same approach as full function but filter for nz=1
  ...
  % Filter for nz == 1 only
  nz1_condition = (nz == 1);
  symvar_nz1 = symvar & nz1_condition;
  
  [~,tilden2reci] = tildentensor(nx,ny,nz,Dw(1));
  JM=length(find(symvar_nz1));
  JJ=altzeros([JM,JM],Dw(1));
  [nsymvar,NN]=makeindices(N,nx,ny,nz,nt,symvar); 
  
  % Create mapping from full symvar indices to nz1 symvar indices
  full_symvar_indices = find(symvar);
  nz1_symvar_indices = find(symvar_nz1);
  [~, nz1_row_mapping] = ismember(nz1_symvar_indices, full_symvar_indices);
  
  compsymvar=comp(symvar);
  compsymvar_nz1=comp(symvar_nz1);
  compind{1}=(compsymvar_nz1==1);
  compind{2}=(compsymvar_nz1==2);
  compind{3}=(compsymvar_nz1==3);

  % for loop to fill the columns of the matrix - only iterate over nz == 1 elements
  for jjj=find(symindex & nz1_condition)'      
    ...
    % will lead to a nonvanishing contribution, AND only nz=1 elements
    jj=(abs(nsx)<=N(1) & abs(nsy)<=N(2) & abs(nsz)<=N(3) & abs(nst)<=N(4)) & (nsymvar(:,3) == 1);
