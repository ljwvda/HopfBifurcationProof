...
load('eigvec_secondHopf.mat','eigvec1');
v1 = eigvec1;
Omega = 2.5629;  % Imaginary part of eigenvalue at Hopf bifurcation
nu = 0.21;      % Viscosity parameter
...
N_initial = [30, 30, 1, 0];  % Modes: nonzero at nz=1 or nz=-1 only
solshape.type = 'rec';
...
% Change N
N=[Nbig,Nbig,0,0]; % nonzero elements are at nz=1 and nz=-1
solshape.type='rec';
solshape.Nrec=N;
...
% The function will filter to nz=1 components and use the 3D ellips shape for Jacobian
Glarge = Gfunc_Hopf_nz1(rv1large_ell3D,iv1large_ell3D,Omega,nu,wTrlarge_ell3D,wTilarge_ell3D,sizeshape_Hopf(Edaggershape3D),symmetry,Edaggershape3D);
disp(['Norm of Glarge (Edaggershape3D) is ',num2str(altsup(norm(Glarge)))]);

DGext = DGfunc_Hopf_nz1(rv1large_ell3D,iv1large_ell3D,Omega,nu,wTrlarge_ell3D,wTilarge_ell3D,sizeshape_Hopf(Edaggershape3D),symmetry,Edaggershape3D);
...
%% Parallel version
Z1finite_parallel = Z1finitebound_G_parallel_nz1(Aext,rv1large_Q2N,iv1large_Q2N,Omega,nu,wTrlarge_Q2N,wTilarge_Q2N,symmetry,Edaggershape3D,N,Etildeshape,eta,etaPhase);
