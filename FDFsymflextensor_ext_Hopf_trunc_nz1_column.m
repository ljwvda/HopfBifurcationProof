function [J_col] = FDFsymflextensor_ext_Hopf_trunc_nz1_column(x,nu,~,~,solshape,symmetry,Jshape,col_idx)
% Computes a single column of the Jacobian for nz==1 elements only
% 
% This is an optimized version of FDFsymflextensor_ext_Hopf_trunc_nz1 that computes only
% the specified column of the Jacobian matrix without building the full
% matrix.
%
% Inputs are the same as FDFsymflextensor_ext_Hopf_trunc_nz1, plus:
...
J_col = Dnonlinear_nz1_column(w,Dw,Mw,DMw,N,Jshape,idx_symmetry,col_idx);
...
function J_col=Dnonlinear_nz1_column(w,Dw,Mw,DMw,N,Jshape,symmetry,col_idx)
...
% FOR NZ==1 ONLY
  ...
  % Create the inverse mapping: from full symvar positions to nz1 positions
  full_to_nz1_mapping = zeros(length(full_symvar_indices), 1);  
  for i = 1:length(nz1_row_mapping)
    if nz1_row_mapping(i) > 0
      full_to_nz1_mapping(nz1_row_mapping(i)) = i;
    end
  end
  
  compsymvar=comp(symvar);
  
  % Loop over all jjj that have nz=1 (same as full function)
  for jjj = find(symindex & nz1_condition)'
    ...
    % Map to nz1-only indices and add contributions to the column
    jj_positions = find(jj);
    row_indices = full_to_nz1_mapping(jj_positions);
